...
Wallet.prototype.createTx = function(to, value, fee, minConf, unspents) {
  ...
  var idx; var finalIdx; var nIdx; var endIdx; var tempUnspents
  var splitUnspents = []; var builders = []; var accums = []; var fees = [];
  var tmpBuilder; var sumAccums; var sumFees
  ...
  utxos.some(function(unspent) {
    ...
    if (value < network.dustSoftThreshold) {
      ...
      */
      idx = getIdx(unspents.length, maxUnspents)
      ...
      */
      ...
      for (var i = 0; i <= finalIdx; i++) {
        ...
        */
        ...
        tempUnspents = shuffledUTXO.slice(idx[i], endIdx)
      ...
      if (sumAccums >= subTotal) {
        ...
        if(change > network.dustSoftThreshold) {
          ...
          builder[argmax(accums)].addOutput(that.getNextChangeAddress(), change)
        }
        return true
      }
    } else {
