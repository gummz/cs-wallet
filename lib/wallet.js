...
bitcoin.networks = _.merge(bitcoin.networks, {
  ...
  smileycoin: {
    ...
    dustSoftThreshold: 0,
    feePerKb: 100000,
    maxUnspents: 2 // How many unspent inputs can be used in a single transaction
  }
});
...
Wallet.prototype.createTx = function(to, value, fee, minConf, unspents) {
  ...
  var builders = []
  // How much is left of the value to be sent
  // Accumulated value is deducted from this variable
  // everytime the number of unspents hits 200
  var valueLeft = value
  ...
  while(valueLeft > 0) {
    ...
    utxos.some(function(unspent) {
      ...
      // 200 skilgreint annars staðar
      // byrja upp a nytt -- geyma accum
      // þa vinnur maður með annað value

      // Geta verið margir staðir sem nota createTx
      // þetta fall skilar builder.build(), þyrfti þa að skila lista
      // af builder.build() ?
  
      // Þarf að gerast i confirm-send -- langeinfaldast!
  
      accum += unspent.value
      subTotal = valueLeft + estimatedFee
      if (accum >= subTotal) {
      ...
      /* 
      One example
       Transaction with 401 inputs

       At input 200:
       Accumulated value is what will be sent in those 200 inputs.
       "subTotal" becomes: accum + fee.

       Now that the accumulated value has been sent, we need to deduct it from "value".

       STOP looping through utxos.some. Continue with sending this transaction
       with 200 inputs.

       The while loop begins anew, with a reset accum variable, and deducted value variable.

       (Will need to address what happens if only dust remains to send)
      */ 
      ...
      // i: Index of unspent we are at
      // We need to track this, because we don't want to reuse unspents
      i += 1
    })
    // After utxo.some has terminated in this instance of the while loop:
    // Erase item we will never use again
    // Otherwise, when we start a new instance of the while loop,
    // we will be testing UTXOs we are already going to spend in a previous transaction.
    utxos = utxos.slice(i)
    // Since utxo list has been sliced, we start at the beginning of that list.
    i = 0
  ...
  // List of builder.build() -- usually only one transaction.
  // If more than one transaction -- let parent know via callback?
  // "This transaction is too large. Do you want to split it into smaller transactions?"
  // ^ This would happen where we call createTx and get a list of more than one item back.
  return builders
}
