...
Wallet.prototype.createTx = function(to, value, fee, minConf, unspents) {
  ...
  var builders = []; var values = []; var fees = []
    ...
    */
    ...
    idx.forEach(function(index, i) {
      ...
      */
      ...
      var tempUnspents = unspents.slice(index, endIdx)
      accum = utxoValues(tempUnspents)
      values.push(accum)

      validate.preCreateTx(to, values[i], network)

      builder = newBuilder(to, tempUnspents, new bitcoin.TransactionBuilder(network), that)
  ...
  // Index of transaction which will pay for the change
  // We want this to be the transaction with the highest value
  var changePayer = argmax(values)
  values[changePayer] -= change

  builders.forEach(function(builder, i) {
    validate.postCreateTx(values[i], values[i]-fees[i], this.getBalance(), fees[i])
    if (values[i] < network.dustSoftThreshold) {
    ...
    builder.addOutput(to, values[i])
    if (i == changePayer && change > network.dustSoftThreshold) {
      builder.addOutput(that.getNextChangeAddress(), change)
    }
    builder.build()
  })
...
function estimateFeePadChangeOutput(tx, network, feePerKb) {
  ...
  return toAtom(Math.ceil(toUnit(fee)))
}
