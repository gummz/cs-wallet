...
Wallet.prototype.createTx = function(to, value, fee, minConf, unspents) {
      ...
      */
      ...
      for (var i = 0; i <= finalIdx; i++) {
        ...
        */
        ...
        var tmpBuilder = newBuilder(
          ...
          that)
        /*
        Since the builder is missing the output when it's sent to estimateFeePadChangeOutput,
        we supply the recipient address.
        */
        fees.push(
          estimateFeePadChangeOutput(
            tmpBuilder.buildIncomplete(), network, network.feePerKb, to)     
        )   
        /*
        ...
        */
        tmpBuilder.addOutput(to, accums[i]-fees[i])
        builders.push(tmpBuilder)
      }
      ...
      if (sumAccums >= subTotal) {
        ...
        if(change > network.dustSoftThreshold) {
          ...
          tmpBuilder[argmax(accums)].addOutput(that.getNextChangeAddress(), change)
        }
        return true
      }
    } else {
  ...
  validate.postCreateTx(value, sumAccums, this.getBalance(), sumFees)
...
function estimateFeePadChangeOutput(tx, network, feePerKb, to) {
  var tmpTx = tx.clone()
  try {
    ...
    So we create a dummy output here.
    */
    tmpTx.addOutput(to, 1)
    tmpTx.addOutput(tx.outs[0].script, network.dustSoftThreshold || 0)
  }
