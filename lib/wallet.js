...
Wallet.prototype.createTx = function(to, value, fee, minConf, unspents) {
  ...
  if (this.networkName === 'bitcoincash') {
    ...
    */
    ...
    ^ Transaction is split at input 55 and 105, so the transactions become
    ...
    */
    ...
    idx.forEach(function(index, i) {
      var tempUnspents = unspents.slice(idx[index-1],index)
  ...
  builders.forEach(function(builder, i) {
    ...
    validate.postCreateTx(values[i],values[i]-fee[i],)
    builder.build()
  })
